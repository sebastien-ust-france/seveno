...
function AnswerField({ snapshot, state, disabled, onChange }: {
  ...
  return <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
    ...
    {state.source === 'reusable_profile' && !state.confirmed ? <p className="mt-3 text-sm text-cyan-200">Réponse déjà renseignée - confirmez qu’elle est toujours exacte.</p> : null}
    <div className="mt-4">
      {snapshot.answerType === 'boolean' ? <Select disabled={disabled} value={state.answerValue === null ? '' : state.answerValue ? 'true' : 'false'} onChange={(event) => setValue(event.target.value === '' ? null : event.target.value === 'true')}><option value="">Sélectionner</option><option value="true">Oui</option><option value="false">Non</option></Select> : null}
      {(snapshot.answerType === 'single_choice' || snapshot.answerType === 'level') ? <Select disabled={disabled} value={typeof state.answerValue === 'string' ? state.answerValue : ''} onChange={(event) => setValue(event.target.value || null)}><option value="">Sélectionner</option>{snapshot.options.map((option) => <option key={option.value} value={option.value}>{option.candidateLabel}</option>)}</Select> : null}
    ...
    <div className="space-y-6">
      ...
      {offer ? <>
        ...
        {!application ? <SevenoPanel tone="blue"><h2 className="text-xl font-semibold text-white">Candidater à cette offre</h2><p className="mt-3 text-sm leading-7 text-slate-300">Vos réponses restent modifiables tant que la candidature n’est pas soumise. Aucune information n’est envoyée à l’entreprise avant la soumission.</p><button type="button" disabled={saving} onClick={() => void begin()} className="mt-5 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 px-5 py-3 text-sm font-semibold text-white disabled:opacity-50">Commencer ma candidature</button></SevenoPanel> : <>
          ...
          <section><h2 className="text-xl font-semibold text-blue-100">Prérequis optionnels - valeur ajoutée</h2><p className="mt-2 text-sm text-slate-300">Les éléments optionnels valorisent votre candidature mais ne sont pas éliminatoires.</p><div className="mt-4 space-y-4">{offer.preferredPrerequisites.map((snapshot) => <AnswerField key={snapshot.prerequisiteCode} snapshot={snapshot} disabled={locked} state={answers[snapshot.prerequisiteCode] ?? { answerValue: null, confirmed: false, source: 'application' }} onChange={(state) => setAnswers((current) => ({ ...current, [snapshot.prerequisiteCode]: state }))} />)}</div></section>
